...
%grinder
resultsPreprocessing.d_particles = 1/1000; %m, size coming out of grinder
...
initSetBase.solution.Ci_Fe3_cell = 1.4;
...
paramSetBase.n_units = 18; %number of anode-cathode surface pairs
...
paramSetBase.LD_bed = 3; %L/d ratio of leaching bed
paramSetBase.vol_lch = paramSetBase.vol_bed-V_PCB_total; %L, volume of electrolyte in bed
tau_lch = 5; %residence time in leaching unit, s
...
initSetPrecious.solution.Ci_Fe3_cell = 0.3;
...
resultsPrecious.practical.pump.head = 3; %m of electrolyte, reasonable assumption value
...
if base_success == 0 || precious_success == 0 %fail
    save(strcat('Simulations\Final\FailedSims\Sim',datestr(clock,'mmddHHMMSS'),'.mat'),'ModelResults');
else %success
    save(strcat('Simulations\Final\Sim',datestr(clock,'mmddHHMMSS'),'.mat'),'ModelResults');
end