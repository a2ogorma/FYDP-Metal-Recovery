...
PsolidPCB = [initSetPreciousFeasible.solidPCB];
Psolution = [initSetPreciousFeasible.solution];
PIsolidPCB = [initSetPreciousInfeasible.solidPCB];
PIsolution = [initSetPreciousInfeasible.solution];
BsolidPCB = [initSetBaseFeasible.solidPCB];
Bsolution = [initSetBaseFeasible.solution];
BIsolidPCB = [initSetBaseInfeasible.solidPCB];
BIsolution = [initSetBaseInfeasible.solution];
resultBase = [resultsBaseFeasible];
resultPrec = [resultsPreciousFeasible];
IresultBase = [resultsBaseInfeasible];
IresultPrec = [resultsPreciousInfeasible];
...
x(3).valsFeasPrec = [paramSetPreciousFeasible.Q];
x(3).valsFeasBase = [paramSetBaseFeasible.Q];
x(3).valsInPrec = [paramSetPreciousInfeasible.Q];
x(3).valsInBase = [paramSetBaseInfeasible.Q];
...
x(5).valsFeasPrec = [paramSetPreciousFeasible.tfinal]./3600;
x(5).valsFeasBase = [paramSetBaseFeasible.tfinal]./3600;
x(5).valsInPrec = [paramSetPreciousInfeasible.tfinal]./3600;
x(5).valsInBase = [paramSetBaseInfeasible.tfinal]./3600;
x(5).valsAllPrec = [x(5).valsFeasPrec x(5).valsInPrec];
x(5).valsAllBase = [x(5).valsFeasBase x(5).valsInBase];
x(6).name = 'Cathode length (m)';
x(6).valsFeasPrec = [paramSetPreciousFeasible.length];
x(6).valsFeasBase = [paramSetBaseFeasible.length];
x(6).valsInPrec = [paramSetPreciousInfeasible.length];
x(6).valsInBase = [paramSetBaseInfeasible.length];
x(6).valsAllPrec = [x(6).valsFeasPrec x(6).valsInPrec];
x(6).valsAllBase = [x(6).valsFeasBase x(6).valsInBase];
x(7).name = 'Cathode height (m)';
x(7).valsFeasPrec = [paramSetPreciousFeasible.height];
x(7).valsFeasBase = [paramSetBaseFeasible.height];
x(7).valsInPrec = [paramSetPreciousInfeasible.height];
x(7).valsInBase = [paramSetBaseInfeasible.height];
...
x(8).valsFeasPrec = [paramSetPreciousFeasible.n_units];
x(8).valsFeasBase = [paramSetBaseFeasible.n_units];
x(8).valsInPrec = [paramSetPreciousInfeasible.n_units];
x(8).valsInBase = [paramSetBaseInfeasible.n_units];
...
x(9).valsFeasPrec = [paramSetPreciousFeasible.vol_bed];
x(9).valsFeasBase = [paramSetBaseFeasible.vol_bed];
x(9).valsInPrec = [paramSetPreciousInfeasible.vol_bed];
x(9).valsInBase = [paramSetBaseInfeasible.vol_bed];
...
x(10).valsFeasPrec = [paramSetPreciousFeasible.V_app];
x(10).valsFeasBase = [paramSetBaseFeasible.V_app];
x(10).valsInPrec = [paramSetPreciousInfeasible.V_app];
x(10).valsInBase = [paramSetBaseInfeasible.V_app];
...
x(13).valsFeasPrec = [paramSetPreciousFeasible.vol_cell];
x(13).valsFeasBase = [paramSetBaseFeasible.vol_cell];
x(13).valsInPrec = [paramSetPreciousInfeasible.vol_cell];
x(13).valsInBase = [paramSetBaseInfeasible.vol_cell];
...
x(16).name = 'Number of stages in parallel';
...
x(17).name = 'Leaching vessel L/D ratio';
x(17).valsFeasBase = [paramSetBaseFeasible.LD_bed];
x(17).valsInBase = [paramSetBaseInfeasible.LD_bed];
x(17).valsAllBase = [x(17).valsFeasBase x(17).valsInBase];
for n = 1:1:1154
    m_plated(n,:) = resultsBase(n).electrowinning.m_plated(end,:) - resultsBase(n).electrowinning.m_plated(1,:);
    m_PCB_i(n,:) = resultsBase(n).PCB.massRem(1,2:7);
    m_PCB(n,:) = resultsBase(n).PCB.massRem(end,2:7);
    if ModelResults(n).ModelResults.resultsEconomic.metrics.paybackPeriod > 10 | ModelResults(n).ModelResults.resultsEconomic.metrics.paybackPeriod < 0 | ModelResults(n).ModelResults.resultsEnvironmental.metrics.carbonIntensity > 0.411
        im_plated(n,:) = resultsBase(n).electrowinning.m_plated(end,:) - resultsBase(n).electrowinning.m_plated(1,:);
        im_PCB_i(n,:) = resultsBase(n).PCB.massRem(1,2:7);
        im_PCB(n,:) = resultsBase(n).PCB.massRem(end,2:7);
    else
        fm_plated(n,:) = resultsBase(n).electrowinning.m_plated(end,:) - resultsBase(n).electrowinning.m_plated(1,:);
        fm_PCB_i(n,:) = resultsBase(n).PCB.massRem(1,2:7);
        fm_PCB(n,:) = resultsBase(n).PCB.massRem(end,2:7);
    end
end
im_lch = im_PCB_i - im_PCB;
ipct_lch1 = 100*im_lch./im_PCB_i;
ipct_rec1 = 100*im_plated./im_PCB_i;
fm_lch = fm_PCB_i - fm_PCB;
fpct_lch1 = 100*fm_lch./fm_PCB_i;
fpct_rec1 = 100*fm_plated./fm_PCB_i;
m_lch = m_PCB_i - m_PCB;
pct_lch1 = 100*m_lch./m_PCB_i;
pct_rec1 = 100*m_plated./m_PCB_i;
newyy = [];
newyyI = [];
for k = 1:length(fpct_rec1)
    if fpct_rec1(k,1) > 0
        newyy = [newyy fpct_rec1(k,1)];
    end
end
for k = 1:length(ipct_rec1)
    if ipct_rec1(k,1) > 0
        newyyI = [newyyI ipct_rec1(k,1)];
    end
end
yy = newyy;
yyI = newyyI;
...
for j = 1:1:size(x,2)
    ...
    posit = 0.55*range(xballval)+min(xballval);
    text(posit,90,strcat('y = ',' ',num2str(hslope,2),'*x + ',' ',num2str(hyint,2)),'FontSize',15)
    hold on
