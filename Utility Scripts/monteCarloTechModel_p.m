...
for run = 1:1:sims
    ...
    try
        ...
        if resultsPrecious.t(end)<(12*3600)
            error('Shit dont work')
        end
        SuccessVals(run) = 1;
        save(strcat("./SuccessRuns/Precious/Run",datestr(clock,'mmddHHMMSS'),".mat"),'resultsPrecious')
    catch
        SuccessVals(run) = 0;
        save(strcat("./FailRuns/Precious/Run",datestr(clock,'mmddHHMMSS'),".mat"),'paramSetPrecious','initSetPrecious')
    end
end
toc
