...
sim_files = dir(fullfile('Simulations\Sensitivity\Dp1-10', '*.mat'));
ModelResults = open(fullfile('Simulations\Sensitivity\Dp1-10',sim_files(1).name));
for k = 2:1:length(sim_files)
    ModelResults(k) = open(fullfile('Simulations\Sensitivity\Dp1-10',sim_files(k).name));
end
...
scatter(([resultsPreprocessing.d_particles]*1000),[y.netAnnualafterTax])
