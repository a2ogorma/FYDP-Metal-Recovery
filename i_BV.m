...
function i_f = i_BV(overpot, i0, iLa, iLc, a, z, temp)
    R = 8.314; %J/mol/K
    F = 96485.3329; %C/mol e-
    j_BV = zeros(1,numel(overpot));
    for idx = 1:1:numel(overpot)
        if (overpot(idx)<0.15 && overpot(idx)>-0.15)||(overpot(idx)>0 && iLa(idx) == -1)||(overpot(idx)<0 && iLc(idx) == -1)
        ...
        elseif overpot(idx)>=0.15
            ...
            rhs = exp(log(i0(idx)/iLa(idx)/z(idx))+a(idx)*z(idx)*F*overpot(idx)/(R*temp));
            j_BV(idx) = iLa(idx)*rhs/(1+rhs);
        elseif overpot(idx)<=-0.15
            %cathodic mass-transfer kinetic mixed case, Tafel approx
            rhs = exp(log(i0(idx)/iLc(idx)/z(idx))-(1-a(idx))*z(idx)*F*overpot(idx)/(R*temp));
            j_BV(idx) = -(iLc(idx)*rhs)/(1+rhs); %note that iL currently always positive
        end
    end
    i_f = j_BV;
end