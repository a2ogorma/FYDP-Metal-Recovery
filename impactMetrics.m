function [resultsEnvironmental, resultsEconomic] = impactMetrics(resultsPreprocessing, resultsBase, resultsPrecious)
    ...
    resultsEnvironmental.energy.EWbase = 8760*resultsPreprocessing.CF*resultsBase.init.paramSet.n_units*resultsBase.numberUnits*(1/resultsBase.init.paramSet.tfinal)*trapz(resultsBase.t,(resultsBase.electrowinning.V_calc.*resultsBase.electrowinning.I_calc))/(PF*1000); %in kWh
    resultsEnvironmental.energy.EWprecious = 8760*resultsPreprocessing.CF*resultsPrecious.init.paramSet.n_units*resultsPrecious.numberUnits*(1/resultsPrecious.init.paramSet.tfinal)*trapz(resultsPrecious.t,(resultsPrecious.electrowinning.V_calc.*resultsPrecious.electrowinning.I_calc))/(PF*1000); %in kWh
    ...
    %Waste disposal
    wastewaterDisposal = 0.0035;%dummy val, $/L
