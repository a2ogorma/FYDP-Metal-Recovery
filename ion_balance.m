function dC_dt = ion_balance(t, C)
    %t = time span (s)
    ...
    solver = @(x) cell_solver(x(1), x(2), x(3), V_app, r_sol, r_hardware, Erev_Fe_cell, Erev_Cu_cell, S_an, S_cat);
    ...
    cor_solver = @(E_corr)cor(E_corr, [Erev_Cu_bed, Erev_Fe_bed]);
