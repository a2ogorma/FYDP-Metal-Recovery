%{
    ...
    Using ode45
    Parameters based on Diaz et al paper 
...
vol_lch = 0.076; %L Volume of bed holding the particles
m_PCB_total = 0.070; %kg Mass of crushed PCBs
...
for j = 1:1:length(t)
    ...
    x0 = [0.2, 0.1, -0.1];
    options = optimoptions(@fsolve, 'Display','off', 'MaxFunctionEvaluations', 6000);
    [x,~,flag,cell_output] = fsolve(solver, x0, options);
    cell_flag(j) = flag;
    if cell_flag(j) <= 0
        disp(cell_output.message)
    end
    I(j) = x(1);
    E_an(j) = x(2);
    E_cat(j) = x(3);
    ...
    [y,~,flag,cor_output] = fsolve(cor_solver, j0, options);
    E_corr(j) = y;
    cor_flag(j) = flag;
    if cor_flag(j) <=0
        disp(cor_output.message);
    end
